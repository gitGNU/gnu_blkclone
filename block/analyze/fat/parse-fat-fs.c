...
void emit_FAT_blocklist(FILE * out,struct FAT_context * ctx, FILE * fs)
{
...
#define FATcell ((step & 1) ? ((cell.odd >> 4) & 0xFFF) : (cell.even & 0xFFF))
      ...
      fread(cell.buf,3,1,fs);
#endif
      fread(cell.buf,3,1,fs);
      while (block < ctx->scount) {
#ifdef DUMP_FAT_INSTEAD
	if (!((block - ctx->ssa) & 15))
	  printf("\n%6X:",block - ctx->ssa);
	printf("%4X",FATcell);
...
#endif
	if (step & 1) fread(cell.buf,3,1,fs);
	step++; block += ctx->spc;
      }
#undef FATcell
    }
...
int main(int argc, char ** argv)
{
  ...
  emit_FAT_blocklist(stdout,&ctx,fs);
